...
const setupActionHandlerMethods = (element: HTMLElement): ActionHandler => {
  ...
  actionHandler.bind = (element: ActionHandlerElement, options: ActionHandlerOptions = {}): void => {
    ...
    element.actionHandler.end = (ev: Event) => {
      ...
      // Prevent the browser's synthetic compatibility mouse events after a
      // touch — without this, touchend is followed by mousedown + click,
      // re-entering end() and firing a second 'tap' on every touch tap.
      if (['touchend', 'touchcancel'].includes(ev.type) && ev.cancelable) {
        ev.preventDefault();
      }
      ...
      if (actionHandler.held) {
        // Consume the flag: a later keyboard click() never runs start(), so a
        // stale hold would otherwise swallow keyboard activation forever.
        actionHandler.held = false;
        if (options.isMomentary) {
          customFireEvent(element, 'action', { action: 'release' });
        }
...
const getActionHandler = (): ActionHandler => {
  ...
  const existing = body.querySelector('.action-handler-fan-remote-card');
  if (existing) {
    return existing as ActionHandler;
  }

  const div = document.createElement('div');
  div.className = 'action-handler-fan-remote-card';
